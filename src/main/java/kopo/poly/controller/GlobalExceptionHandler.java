 ...
 *  전역 예외 처리를 답당하는 클래스
 *
 * @ControllerAdvice를 사용하여 애플리케이션 전체에서 전체에서 발생하는 예외를 중앙에서 처리함.
 */
@RestController
@ControllerAdvice
public class GlobalExceptionHandler {
     ...
     * - 클라이언트에가 '409 Conflict' 상태 코드와 함께 메세지를 반환
     * @param e 발생한 Optimistic LockException 예외 객체
    ...
    public ResponseEntity<CommonResponse<MsgDTO>> handleOptimisticLockException(OptimisticLockException e) {
        ...
        MsgDTO dto = MsgDTO.builder()
                .result(0)
                .msg("다른 사용자가 먼저 변경했습니다. 다시 시도해 주세요. error : " + e.getMessage())
                .build();
    ...
    public ResponseEntity<CommonResponse<String>> handleIllegalAccessException(IllegalAccessException e) {

        return ResponseEntity.ok(
                CommonResponse.of(HttpStatus.BAD_REQUEST, HttpStatus.BAD_GATEWAY.series().name(), e.getMessage()));
    }
    /**
     * 서버 내부 어루(Exception) 발생 시 예외 처리
     * - 예상하지 못한 오류가 발생한 경우 처리
     * - 클라이언트에게 '500 Internal Server Error' 상태 코드와 함께 메세지를 반환
     *
